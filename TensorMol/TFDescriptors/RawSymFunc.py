...
def tf_sym_func_element_codes(xyzs, Zs, pairs, triples, element_codes, radial_gauss, radial_cutoff,
		angular_gauss, thetas, angular_cutoff, zeta, eta):
	"""
	A tensorflow implementation of the AN1 symmetry function for a set of molecule.
	Args:
...
def tf_sym_func_element_codepairs(xyzs, Zs, pairs, triples, element_codes, element_codepairs,
		codepair_idx, radial_gauss, radial_cutoff, angular_gauss, thetas, angular_cutoff, zeta, eta):
	"""
	A tensorflow implementation of the AN1 symmetry function for a set of molecule.
	Args:
		xyzs: nmol X maxnatom X 3 tensor of coordinates.
	...
	angular_embed = tf_angular_sym_func(dtxyzs, angular_gauss, thetas, angular_cutoff, zeta, eta)
	padding_mask = tf.where(tf.not_equal(Zs, 0))
	coded_angular_embed = tf_angular_codepair_channel_sym_func(angular_embed, triples_Zs, element_codepairs,
							codepair_idx, padding_mask, scatter_idx)
	embed = tf.concat([coded_radial_embed, coded_angular_embed], axis=-1)
	return embed

def tf_sym_func_element_codepairs_replace(xyzs, Zs, pairs, triples, element_codes, element_codepairs,
		codepair_idx, radial_gauss, radial_cutoff, angular_gauss, thetas, angular_cutoff, zeta, eta,
		replace_idx, replace_codes, replace_codepairs):
	"""
	A tensorflow implementation of the AN1 symmetry function for a set of molecule.
	Args:
		xyzs: nmol X maxnatom X 3 tensor of coordinates.
	...
	coded_radial_embed = tf_radial_code_channel_sym_func_replace(radial_embed, pair_Zs, element_codes,
						replace_codes, pairs_replace_mask)
	dtxyzs, triples_Zs, scatter_idx = sparse_triples(xyzs, Zs, triples)
...
def tf_angular_sym_func(dtxyzs, angular_gauss, thetas, angular_cutoff, zeta, eta):
	"""
...
def tf_angular_sym_func_v2(dtxyzs, angular_gauss, thetas, angular_cutoff, zeta, eta):
	...
	May be more stable when training with atomic force gradients.
	...
	cos_factor = 2.0 * tf.square(tf.expand_dims(cos_angle, axis=-1) * cos_thetas
				+ tf.expand_dims(sin_angle, axis=-1) * sin_thetas)
	exponent = tf.expand_dims(tf.reduce_sum(dist_jk_tensor, axis=-1) / 2.0, axis=-1) - angular_gauss
	dist_factor = tf.exp(-eta * tf.square(exponent))
	cutoffj = 0.5 * (tf.cos(np.pi * dist_jk_tensor[...,0] / angular_cutoff) + 1.0)
	cutoffk = 0.5 * (tf.cos(np.pi * dist_jk_tensor[...,1] / angular_cutoff) + 1.0)
...
def tf_radial_code_channel_sym_func(radial_embed, pair_Zs, element_codes):
	pair_codes = tf.gather(element_codes, pair_Zs)
	pad_mask = tf.where(tf.equal(pair_Zs, 0), tf.zeros_like(pair_Zs, dtype=eval(PARAMS["tf_prec"])),
				tf.ones_like(pair_Zs, dtype=eval(PARAMS["tf_prec"])))
...
def tf_radial_code_channel_sym_func_replace(radial_embed, pair_Zs, element_codes, replace_codes, replace_mask):
	pair_codes = tf.gather(element_codes, pair_Zs)
	pair_codes = tf.where(tf.tile(tf.expand_dims(replace_mask[0,...], axis=-1), [1, 1, 4]),
			tf.tile(tf.reshape(replace_codes, [1, 1, -1]),
			[tf.shape(pair_codes)[0], tf.shape(pair_codes)[1], 1]), pair_codes)
	pad_mask = tf.where(tf.equal(pair_Zs, 0), tf.zeros_like(pair_Zs, dtype=eval(PARAMS["tf_prec"])),
				tf.ones_like(pair_Zs, dtype=eval(PARAMS["tf_prec"])))
	pair_codes *= tf.expand_dims(pad_mask, axis=-1)
	radial_embed = tf.expand_dims(radial_embed, axis=-2) * tf.expand_dims(pair_codes, axis=-1)
...
def tf_angular_code_channel_sym_func(angular_embed, triples_Zs, element_codes, padding_mask, triples_scatter):
...
def tf_angular_codepair_channel_sym_func(angular_embed, triples_Zs, element_codepairs, codepair_idx,
		padding_mask, triples_scatter):
	...
	scatter_shape = [tf.shape(padding_mask)[0], tf.reduce_max(triples_scatter[:,1]) + 1, tf.shape(element_codepairs)[1], 8, 8]
	angular_embed = tf.reduce_sum(tf.scatter_nd(triples_scatter, angular_embed, scatter_shape), axis=1)
	angular_embed = tf.reshape(angular_embed, [tf.shape(angular_embed)[0], tf.shape(element_codepairs)[1], -1])
	return angular_embed

def tf_angular_codepair_channel_sym_func_replace(angular_embed, triples_Zs, element_codepairs, codepair_idx,
		padding_mask, triples_scatter, replace_idx, replace_codepairs, replace_mask, replace_Z):
	codepair_gather = tf.gather_nd(codepair_idx, triples_Zs)
	codepairs = tf.gather(element_codepairs, codepair_gather)
	other_triples = tf.where(tf.equal(triples_Zs[:,0], replace_Z), triples_Zs[:,1], triples_Zs[:,0])
	gather_replace = tf.stack([tf.tile(tf.range(tf.shape(replace_idx)[0], dtype=tf.int32),
					[tf.shape(other_triples)[0]]), other_triples], axis=-1)
