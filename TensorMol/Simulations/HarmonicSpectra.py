...
def HarmonicSpectra(f_, x_, at_, grad_=None, eps_ = 0.001, WriteNM_=False, Mu_ = None, Temp_=300.0, h_=None):
	"""
	...
	# At 300K in Hartrees
	KbT = KAYBEETEE*(Temp_/300.0)
	RT = AVOCONST*KbT
	R = AVOCONST*KAYBEETEE/300.0
	VibEs = np.sqrt(abs(w))
	ZeroPointEnergy = np.sum(VibEs)/2.0
	Denom = np.exp(np.sqrt(abs(w+1.0e-18))/KbT-1.0)
	VibEnthalpy = ZeroPointEnergy+np.sum(VibEs/(Denom))
	VibEntropy = np.sum((VibEs/KbT)/Denom - np.log(1.0-1.0/Denom))

	# Compute moments of intertia.
	...
	# Ideal Gas Translational Enthalpy and Entropy at 1 atm.
	Pressure = 1.0
	TotalMass = np.sum(m_)
	TransEnthalpy = (3.0/2.0)*AVOCONST*KbT
	TransEntropy = R*np.log( np.power(2.0*Pi*TotalMass*KbT,3.0/2.0)*KbT/Pressure + 5.0/2.0)

	TotalEnthalpy = TransEnthalpy + RotEnthalpy + VibEnthalpy + RT
	TotalEntropy = TransEntropy + RotEntropy + VibEntropy

	thermodynamics = {"TotalEnthalpy":TotalEnthalpy,"TransEnthalpy":TransEnthalpy,
						"RotEnthalpy":RotEnthalpy,"VibEnthalpy":VibEnthalpy,
						"TotalEntropy":TotalEntropy,"TransEntropy":TransEntropy,
						"RotEntropy":RotEntropy,"VibEntropy":VibEntropy}
	...
	if (WriteNM_):
		...
		return wave, v, intensities, thermodynamics
	return wave, v, thermodynamics
