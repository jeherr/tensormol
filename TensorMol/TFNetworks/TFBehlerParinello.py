...
class BehlerParinelloGauSHConv(BehlerParinelloGauSH):
	...
	def __init__(self, mol_set_name=None, name=None):
		...
		Notes:
			if name != None, attempts to load a previously saved network, otherwise assumes a new network
		"""
		BehlerParinelloGauSH.__init__(self, mol_set_name, name)
		if name == None:
			self.network_type = "BPGauSH"
			self.name = self.network_type+"_"+self.mol_set_name+"_"+time.strftime("%a_%b_%d_%H.%M.%S_%Y")
			self.network_directory = PARAMS["networks_directory"]+self.name
			...
			for e in range(len(self.elements)):
				...
				for i in range(len(self.filters)):
					if i == 0:
						...
						with tf.name_scope(str(self.elements[e])+'_conv'+str(i+1)):
							conv = tf.layers.conv1d(branches[-1][-1], filters=self.filters[i], kernel_size=self.kernel_size[i],
								strides=self.strides[i], padding="valid", activation=self.activation_function)
							branches[-1].append(conv)
				for i in range(len(self.hidden_layers)):
					if i == 0:
